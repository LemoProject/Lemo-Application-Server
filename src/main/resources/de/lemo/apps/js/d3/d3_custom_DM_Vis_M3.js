		...
		console.log("Bin im Init - Finished Node Init, starting Link Init " +_links.length);
	   	...
	   	if(_links.length){
			...
			$.each(_links, function(i,v) {
				if(v.pathId <= amt*(pages-page+1) && v.pathId > amt*(pages-page)){
					links.push({"source":nodes[v.source-skippedCounter],"target":nodes[v.target-skippedCounter],"value":v.value});
		   			 console.log("Schreibe Link: PathId "+nodes[v.source-skippedCounter].pid+"("+v.source+"--"+v.target+")"+" Source: "+nodes[v.source-skippedCounter].name+" Target: "+nodes[v.target-skippedCounter].name);
				} else console.log("Max. number of pathes ("+amt+") reached ...!");
			});
	   	} else {
