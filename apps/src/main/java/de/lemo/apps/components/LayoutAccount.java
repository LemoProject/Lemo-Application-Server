...
public class LayoutAccount {
	...
	public Integer getCourseAmount(){
		List<Course> userCourses = courseDAO.findAllByOwner(userWorker.getCurrentUser());
		if(userCourses!=null) return userCourses.size();
		else return 0;
	}
	
	public String getActivePage(String navName){
		if(subpage!=null && subpage.equals(navName)) return "active";
